    ...
    def get_returns(self):
        ...
        if isinstance(self.symbols, list) and len(self.symbols) > 1:
            ...
            for symbol in self.symbols:
                returns = pd.DataFrame(yf.download(symbol, self.START_DATE, cal.today())[
            ...
            # calculate weighted returns via matrix multiplication
            df['portf_rtn'] = pd.DataFrame(df.dot(self.weights))
            # drop individual asset returns and keep only weighted portfolio returns
            df = df[[x for x in df.columns if x not in self.symbols]]
