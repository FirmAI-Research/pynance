...
class Returns():
    ...
    def __init__(self, symbols:list=['SPY'], weights:list=None, start_date=Calendar().today(), end_date=None, time_sample='D'):
    ...
    def price_series(self):
        ''' return a series of prices for a list of symbols over a given time period
        must return a pandas dataframe to so handle case when 1 symbol returns as series
        '''
        if len(self.symbols) > 1:
            df = yf.download(self.symbols, self.start_date, self.end_date)['Close']
        else:
            df = pd.DataFrame(yf.download(self.symbols, self.start_date, self.end_date)['Close'])
        return df


    def weighted_returns(self):
        df = self.price_series().pct_change()[1:] # ignore date column?
        weighted_returns = (self.weights * df)
        weighted_returns['weighted_returns'] = weighted_returns.sum(axis=1)
        return weighted_returns


    def cumulative_return(self, df):
        '''Calculate the cumulative return of price series, resampled by -
        '''
        return yf.download(self.symbols, self.start_date, self.end_date)['Close'].resample(self.time_sample).last().pct_change()
