...
class MRVectorBacktester(MomVectorBacktester):
    ...
    def plot_distance(self, threshold):
        self.results['distance'].dropna().plot(figsize=(10,6), legend = True)
        plt.axhline(threshold, color ='y')
        plt.axhline(-threshold, color = 'y')
        plt.axhline(0, color = 'y')
        plt.show()
        print(self.results)
        self.results['position'].plot(ylim=[-1.1,1.1], figsize=(10,6))
        plt.show()
...
if __name__ == '__main__':
    ...
    mrbt.plot_results()
    mrbt.plot_distance(threshold=7.5)
