...
class ScikitVectorBacktester(object):
    ...
    def run_strategy(self, start_in, end_in, start_out, end_out, lags=3):
        ...
        hits = np.sign(self.data_subset['returns'].iloc[lags:] *
                self.data_subset['prediction'].iloc[lags:]
                ).value_counts()
        print(hits)

        accuracy_score = (self.data_subset['prediction'],
                np.sign(self.data_subset['returns']))
    ...
    def plot_results(self):
        ...
        # print(self.data_subset.columns)
        # self.data_subset.plot(x='returns', y=[ 'prediction', 'strategy',],
